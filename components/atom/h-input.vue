    ...
    setup(props, {emit, attrs}) {
      ...
      const isStepUpAllowed = computed(() => {
        return (
          ...
          (props.max === null ||
            parseInt(String(props.modelValue), 10) < props.max)
        )
      })
      ...
      const isStepDownAllowed = computed(() => {
        return (
          props.disabled === false &&
          (props.min === null ||
            parseInt(String(props.modelValue), 10) > props.min)
        )
      })
      ...
      function processValue(event: KeyboardEvent) {
        ...
        if (props.slug === true) {
          ...
          const isAllowed =
            slugSafeCharacters.includes(key) || systemKeys.includes(key)
        ...
        if (props.dbSafe === true) {
          const dbSafeCharacters =
            'abcdefghijklmnopqrstuvwxyz01234567890_ '.split('')

          const isAllowed =
            dbSafeCharacters.includes(key) ||
...
<style lang="scss" scoped>
  .v-input {
    ...
    --v-input-border-color-focus: theme('colors.yaprak.500');
    ...
    .input {
      ...
      .step-down {
        ...
        &:hover:not(.disabled) {
          --arrow-color: theme('colors.yaprak.500');
        }
